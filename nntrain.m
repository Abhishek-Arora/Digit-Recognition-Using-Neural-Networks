...
for i=1: size(ttrain)
    temp = zeros(1,10);
    temp( ttrain(i)) = 1;
    ttrainWithEncoding = vertcat(ttrainWithEncoding, temp);
end
...
for epoch=1:MAX_EPOCH
  ...
  for x_i=1:N
    ...
    % TO DO:: fill this in.
    dW2 = zeros(H+1,K);
    dW2 = [ones(size(Z{1}, 1), 1) Z{1}]' * (Z{2} - ttrainWithEncoding(x_i, :) );

    % Hidden layer derivative.
    % Backpropagate error from output layer to hidden layer.
    % TO DO:: fill this in.
    dW1 = zeros(D+1,H);
    dj = (Z{1} .* (1 - Z{1})).*((max(Z{2}) - ttrainWithEncoding(x_i,:)) * NN(2).weights(1:500,:)');
    dW1 = [1,Xtrain(x_i,:)]'* dj;
